...
func TestCompilerScopes(t *testing.T) {
	...
	if compiler.symbolTable.Outer != globalSymbolTable {
		t.Errorf("compiler didnt enclose symbol table")
	}
	compiler.leaveScope()
	if compiler.scopeIndex != 0 {
		t.Errorf("scopeIndex wrong. got=%d, want=%d", compiler.scopeIndex, 0)
	...
	if compiler.symbolTable != globalSymbolTable {
		t.Errorf("compiler didnt restore global symbol table")
	}
	if compiler.symbolTable.Outer != nil {
		t.Errorf("compiler modified global synbol table incorrectly")
	}
...
func testConstants(t *testing.T, expected []interface{}, actual []object.Object) error {
	...
	for i, constant := range expected {
		...
		case []code.Instructions:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testInstructiona failed: %s", i, err)
			}
